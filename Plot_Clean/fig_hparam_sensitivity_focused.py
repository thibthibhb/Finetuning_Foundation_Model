...
class FocusedHPAnalyzer:
    ...
    def run_analysis(self, csv_path: str, output_dir: str):
        ...
        if critical_params:
            print(f"  • {len(critical_params)} hyperparameters show CRITICAL impact (Δκ > 0.02)")
        moderate_params = [hp for hp in sorted_hps if 0.01 < hp[1][0] <= 0.02]
        if moderate_params:
            print(f"  • {len(moderate_params)} hyperparameters show MODERATE impact (0.01 < Δκ ≤ 0.02)")
        ...
        for category, impacts in category_impact.items():
            ...
            print(f"  • {category} parameters: avg Δκ = {avg_impact:.4f} ({len(impacts)} params)")
