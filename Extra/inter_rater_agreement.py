...
for sub in subjects:
    for ses in sessions:
        try:
            ...
            f1 = f1_score(e1, e2, average='macro', zero_division=0)

            # Compute per-class kappa
            unique_labels = sorted(set(e1 + e2))
            subclass_kappas = {'Subject': sub, 'Session': ses}
            for label in unique_labels:
                bin_e1 = [1 if l == label else 0 for l in e1]
                bin_e2 = [1 if l == label else 0 for l in e2]
                subclass_kappas[label] = cohen_kappa_score(bin_e1, bin_e2)
            
            results.append((sub, ses, kappa, accuracy, f1))
            per_class_kappas.append(subclass_kappas)
...
results_df = pd.DataFrame(results, columns=["Subject", "Session", "Cohen_Kappa", "Accuracy", "F1_Score"])
csv_path = os.path.join(base_path, "interrater_agreement_results.csv")
results_df.to_csv(csv_path, index=False)

# Save per-class kappas
per_class_df = pd.DataFrame(per_class_kappas)
per_class_csv_path = os.path.join(base_path, "interrater_agreement_kappa_per_class.csv")
...
per_class_means = per_class_df.drop(columns=['Subject', 'Session']).mean()

print(f"\nSaved overall results to: {csv_path}")
print(f"Saved per-class kappa to: {per_class_csv_path}")
...
print(f"Mean Macro F1-score across all sessions: {mean_f1:.4f}")

print("\nMean Cohen's Kappa per class:")
print(per_class_means.round(4))
