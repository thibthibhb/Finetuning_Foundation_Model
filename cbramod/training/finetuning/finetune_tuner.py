...
def objective(trial, base_params):
    ...
    # Suggest hyperparams
    params.lr = trial.suggest_float("lr", 5e-5, 5e-3, log=True)
    params.weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-3, log=True)
    params.label_smoothing = trial.suggest_float("label_smoothing", 0.0, 0.5, step=0.01)
    params.dropout = trial.suggest_float("dropout", 0.0, 0.5, step=0.05)
    params.batch_size = trial.suggest_categorical("batch_size", [64, 128, 256, 512, 1024]) #512, 1024,
    params.clip_value = trial.suggest_float("clip_value", 0.0, 4, step=0.1)
    ...
    params.data_ORP = trial.suggest_float("data_ORP", 0.5,0.7, step=0.1)
